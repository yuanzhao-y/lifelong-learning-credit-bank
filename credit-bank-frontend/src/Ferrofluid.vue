<template>
  <div
    ...
    :style="mixBlendMode ? { mixBlendMode } as any : undefined"
  />
</template>
...
const initWebGL = () => {
  ...
  cleanup = () => {
    ...
    if (props.mouseInteraction) canvas.removeEventListener('pointermove', onPointerMove);
    ro.disconnect();
    if (canvas.parentElement === container) {
      container.removeChild(canvas);
    }
    
    // Cleanup OGL resources safely
    const prog = program as any;
    const geom = geometry as any;
    const m = mesh as any;
    const rend = renderer as any;
    if (prog && typeof prog.remove === 'function') prog.remove();
    if (geom && typeof geom.remove === 'function') geom.remove();
    if (m && typeof m.remove === 'function') m.remove();
    if (rend && typeof rend.destroy === 'function') rend.destroy();
  };
};
...
// Watch for prop changes to re-initialize or update uniforms
watch(() => [
  props.dpr,
  props.paused,
  props.colors,
